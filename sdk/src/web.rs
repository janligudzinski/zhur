...
impl<S: Serialize> Responder for Json<S> {
    fn modify_response(self, res: &mut HttpRes) {
        res.body = HttpBody::Text(serde_json::to_string(&self.0).unwrap());
        res.headers
...
impl Responder for File {
    fn modify_response(self, res: &mut HttpRes) {
        res.body = HttpBody::Binary(self.0);
        res.headers.insert(
            "Content-Type".to_owned(),
            self.1.unwrap_or("application/octet-stream".to_owned()),
        );
    }
}
impl File {
