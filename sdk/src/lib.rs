...
macro_rules! text_function {
    ($function_name:ident) => {
        fn func_wrapper(msg: &[u8]) -> zhur_sdk::wapc_guest::CallResult {
            let msg_string = zhur_sdk::bincode::deserialize::<String>(msg)
                .expect("a text function expects a deserializable utf-8 string");
            let output = $function_name(msg_string);
            let response = zhur_sdk::bincode::serialize(&output)
        ...
        pub fn wapc_init() {
            ...
            zhur_sdk::wapc_guest::register_function("text", func_wrapper)
        }
    };
}
